...
def accuracy(typed, source):
    ...
    words_source = split(source)
    count = 0
    for i in range(len(words_typed)):
        if i >= len(words_source):
            break
        if words_typed[i] == words_source[i]:
            count += 1
    return 100.0 * (count / len(words_typed))
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    min_idx = 0
    min_diff = 1000 # 一个单词几乎不可能又1000个字母组成
...
def furry_fixes(typed, source, limit):
    ...
    def helper(typed, source, count):
        ...
        if len_typed == 0 or len_source == 0:
            return count + len_typed + len_source
        if typed[0] != source[0]:
            return helper(typed[1:], source[1:], count+1)
...
def minimum_mewtations(typed, source, limit):
    ...
    def helper(typed, source, count):
        ...
        if typed == "" or source == "": # Base cases should go here, you may add more base cases as needed.
            # BEGIN
            "*** YOUR CODE HERE ***"
            return count + len(typed) + len(source)
            # END
        ...
        if typed[0] == source[0]: # Feel free to remove or add additional cases
            ...
            add = helper(source[0] + typed, source, count+1)
            remove = helper(typed[1:], source, count+1)
            substitute = helper(source[0] + typed[1:], source, count+1)
            # BEGIN
            "*** YOUR CODE HERE ***"
...
def fastest_words(words_and_times):
    ...
    for word_idx in word_indices:
        min_time = get_time(times, 0, word_idx)
        min_player_idx = 0
        for player_idx in range(1, len(times)):
            t = get_time(times, player_idx, word_idx)
            if t < min_time:
                min_time = t
                min_player_idx = player_idx
        result[min_player_idx].append(words[word_idx])
    return result
    # END PROBLEM 10
